...
function buildReasoningPrompt(goal: string, networkName: string): string {
  const capList = knownCapabilities
    .map((c) => `  - ${c.id}: ${c.label}`)
    .join("\n");

  return `You are a Trust Steward agent on ${networkName}. Given a natural-language goal, determine which capabilities are required to accomplish it.
...
function parseReasoning(
  text: string,
  goal: string,
): { reasoning: string; capabilities: CapabilityRef[] } {
  ...
  if (caps.length === 0) {
    ...
    return {
      ...
      capabilities:
        fallback.length > 0 ? fallback : knownCapabilities.slice(3, 5),
    };
  }
...
async function zeroGReason(
  prompt: string,
): Promise<{ text: string; verified: boolean; model: string }> {
  ...
  const provider =
    process.env.ZEROG_PROVIDER || "0x69Eb5a0BD7d0f4bF39eD5CE9Bd3376c61863aE08";
  ...
  if (!computeMetadata) {
    computeMetadata =
      await computeBroker.inference.getServiceMetadata(provider);
  }
  ...
  const verified = await computeBroker.inference.processResponse(
    provider,
    chatID,
    usage,
  );
...
function rpcOverride(networkId: string): string | undefined {
  if (networkId === "atlantic-testnet") return process.env.PHAROS_RPC_URL;
  if (networkId === "monad-testnet") return process.env.LIGIS_MONAD_RPC_URL;
  return undefined;
}

function getWalletClient(ctx: EvmReadContext) {
  const account = getStewardAccount();
  if (!account) return null;
  const rpc = rpcOverride(ctx.networkId) ?? ctx.rpcUrl;
  return {
    client: createWalletClient({
      account,
      transport: http(rpc, { retryCount: 3, timeout: 20_000 }),
      chain: ctx.chain,
    }),
    account,
  };
 ...
 * This bypasses eth_sendTransaction (not supported by some RPCs).
 *
 * On Monad, gas is charged as `gasPrice × gasLimit` (not used gas) — estimate
 * tightly and only pad ~10%.
 */
...
/** Fallback demo subject when no steward key is configured (read-only runs). */
const DEMO_SUBJECT: Record<string, Address> = {
  "atlantic-testnet": "0xd21a4c7ab1a52a2Ab48A6f0271984d5c3D4027Ec",
  "monad-testnet": "0xd21a4c7ab1a52a2Ab48A6f0271984d5c3D4027Ec",
};

export async function* stewardLoop(
  goal: string,
  opts: { live: boolean; clientIp?: string; networkId?: string },
): AsyncGenerator<StewardEvent> {
  const { live } = opts;
  const networkId = opts.networkId ?? DEFAULT_EVM_NETWORK;
  ...
  if (canWrite && opts.clientIp && !checkRateLimit(opts.clientIp)) {
    yield {
      type: "error",
      message: "Rate limit: too many live runs. Try again in a minute.",
    };
    return;
  }

  // Subject address: use steward wallet if available, otherwise a known demo subject.
  const subject =
    wallet?.account.address ??
    DEMO_SUBJECT[networkId] ??
    ("0xd21a4c7ab1a52a2Ab48A6f0271984d5c3D4027Ec" as Address);

  let tokenId = "0";
  let minted = false;

  if (canWrite) {
    const existing = await readAgentId(subject, networkId);
    rpcCalls++;
    if (existing === 0n) {
      const hash = await sendWriteContract(wallet!, ctx, {
      ...
      const newId = await readAgentId(subject, networkId);
      rpcCalls++;
  ...
  if (live && hasZeroG) {
    ...
    try {
      const prompt = buildReasoningPrompt(goal, ctx.chain.name);
  ...
  if (live) {
    ...
    capableResults = await isCapableMulti(subject, hashes, networkId).catch(
  ...
  for (let i = 0; i < requiredCaps.length; i++) {
    ...
    if (canWrite) {
      try {
        ...
        const issueHash = await sendWriteContract(wallet!, ctx, {
          address: addresses.credentialRegistry,
          abi: CREDENTIAL_REGISTRY_ABI,
          functionName: "issue",
          args: [
        ...
        await ctx.client.waitForTransactionReceipt({ hash: issueHash });
  ...
  if (canWrite) {
    ...
    try {
      const anchorHash = await sendWriteContract(wallet!, ctx, {
        address: addresses.pharosAgentId,
        abi: PHAROS_AGENT_ID_ABI,
        functionName: "setTokenURI",
        args: [BigInt(tokenId), tokenUri],
      });
      await ctx.client.waitForTransactionReceipt({ hash: anchorHash });
      anchorTx = anchorHash;
    } catch {
      anchorTx =
