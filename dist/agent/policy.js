 ...
 */
import { capabilityHash } from "../lib/index.js";
/**
...
export function findCapability(nameOrHash) {
    return KNOWN_CAPABILITIES.find((c) => c.name === nameOrHash || c.hash.toLowerCase() === nameOrHash.toLowerCase());
}
// ---------- Reasoning prompt ----------
/**
 ...
 */
export function buildReasoningPrompt(goal) {
    const capList = KNOWN_CAPABILITIES.map((c) => `  - ${c.name}: ${c.description}`).join("\n");
...
export function parseReasoning(text) {
    ...
    const rawCaps = Array.isArray(json.capabilities) ? json.capabilities : [];
