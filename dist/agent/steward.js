import { getAgentId, issueId, signCredential, submitCredential, updateTokenUri, verify, } from "../lib/index.js";
...
export class TrustSteward {
    ...
    reasoner;
    store;
    constructor(ctx, reasoner, store) {
        this.ctx = ctx;
    ...
    async run(goal, opts) {
        ...
        if (!dryRun) {
            const manifest = this.buildManifest(tokenId, controller, goal, reasoning, capResults, gated, txHashes, "");
            try {
        ...
        const finalManifest = this.buildManifest(tokenId, controller, goal, reasoning, capResults, gated, txHashes, anchoredTokenUri);
        return {
            ok: true,
    ...
    buildManifest(tokenId, controller, goal, reasoning, capabilities, gated, txHashes, anchoredTokenUri) {
        return {
            version: 1,
            agentId: tokenId,
            controller,
    ...
    fail(goal, controller, tokenId, minted, txHashes, error) {
        const manifest = this.buildManifest(tokenId, controller, goal, { text: "", verified: false, model: "", provider: "" }, [], false, txHashes, "");
        return {
            ok: false,
            booted: { tokenId, minted },
