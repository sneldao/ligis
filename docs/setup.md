...
| `CROO_SERVICE_ID_LIGIS_ISSUE`      | CROO provider                                      | Listing UUID for `ligis.issue` — also returned by a `ligis.risk` `pathToTrust` hint                                                                                                                                   |
| `CROO_SERVICE_ID_LIGIS_GATE`       | CROO provider                                      | Listing UUID for `ligis.gate`                                                                                                                                                                                         |
| `CROO_SERVICE_ID_LIGIS_QUALIFY`    | CROO provider                                      | Listing UUID for `ligis.qualify`                                                                                                                                                                                      |
| `LIGIS_SELF_ISSUABLE_CAPABILITIES` | CROO `ligis.issue` + `ligis.qualify`               | Comma-separated capabilities mintable without external evidence. Keep empty in production — a buyable `kyc.basic` makes the credential meaningless. Legacy `LIGIS_QUALIFY_SELF_ISSUABLE` is still read as a fallback. |
| `LIGIS_ISSUER_PRIVATE_KEY`         | CROO `ligis.issue` + `ligis.qualify`               | Issuer key used to sign credentials. Both services refuse to mint a non-allowlisted capability without external evidence (`LIGIS_EAS_*`).                                                                             |
| `LIGIS_JEV_ENABLED`                | x402 gate + CROO `ligis.gate`                      | `1` turns on the Jev payment-intent layer (default off — opt-in)                                                                                                                                                      |
| `AI_GATEWAY_API_KEY`               | Jev, gateway route                                 | Vercel AI Gateway key. Free through **2026-09-25**; attach billing/BYOK after, or move to the direct route.                                                                                                           |
| `TYPESAFE_API_KEY`                 | Jev, direct route                                  | TypeSafe key (console.typesafe.ai). Pair with `LIGIS_JEV_TRANSPORT=direct` once the gateway promo ends.                                                                                                               |
| `LIGIS_JEV_TRANSPORT`              | Jev                                                | `gateway` \| `direct` \| `auto` (default auto). Verify with `pnpm smoke:jev`.                                                                                                                                         |
...
- **Homepage (`/`)**: editorial landing — chain-specific live stats, the gate
  demo, and a framed invite into the field. No live WebGL; the wheel scrolls
  the page.
- **Field (`/field`)**: dedicated immersive registry. Pan/zoom/pinch the agent
  field. Esc or the Ligis mark returns to `/`. Semantic zoom keeps markers
  visible when pulled back (it must not fade to empty paper).
